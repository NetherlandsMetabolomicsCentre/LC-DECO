...
%d = dn; % Not a solution, but to avoid the negativity caused by PCA remodeling
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!%
...
while niter < nit
    ...
    % ********************************
    abss=conc\d;
    %abss = abs(abss); % This is to prevent negative initialisation to the FNN-ALS. This is not known if this is the best solution %
...
for i =1:matc
    ...
    for j=kinic:kfin
        if cneg(i,:) == ones(1,size(isp,2))
            [x,w]=deco_fnnls(abss*abss',abss*d(j,:)');
            conc2(j-kinic+1,:)=x';
        end
    end
    conc(kinic:kfin,:) = conc2;
...
for i = 1:matr
    ...
    for j=kinic:kfin,
        if spneg(:,i)== ones(size(isp,2),1)
            [abss2(:,j-kinic+1),w]=deco_fnnls(conc'*conc,conc'*d(:,j));
        end
    end

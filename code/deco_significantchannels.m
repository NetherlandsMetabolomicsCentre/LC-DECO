function sc = deco_significantchannels(x,RESIDUE_LIMIT)

sc = deco_pca(x,RESIDUE_LIMIT);

end

function usemasses = deco_pca(data,residuelimit)
...
rv = zeros(last,1);
for i=last:-1:1
    rv(i) = deco_rvcoeff(data,data(:,mindex(i:last)));
    if(rv==0)
        msgbox('No significant information in block','warn');
        usemasses=[];
        break;
    end
    if(rv(i)>=1-residuelimit) % cutoff has to be statistically defined; to be implemented
